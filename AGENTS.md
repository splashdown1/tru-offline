...
| `TRU_APEX7.html` | 87.1 MB | 63,337 | **Current canonical.** APEX6 + (1) commentary routing fixed — proper step 0c in override, requires explicit prefix ('commentary on'/'notes on'/'explain'); (2) chapter-level commentary lookup ('notes on psalm 23' → full chapter commentary); (3) bare verse refs don't trigger commentary ('john 3:16' stays SCRIPTURE). 7 routing paths: TRUTH/SCRIPTURE/DEFINE/STRONGS/COMMENTARY/CALC/GAP. 11/13 test suite pass (2 'failures' are correct doctrine-first behavior). || `TRU_APEX_LITE.html` | 44.8 MB | 63,337 | **Phone/mobile build.** APEX7 without commentary layer (no commentary-data block, no commentaryQuery/getCommentary). Dictionary trimmed from 147,982 to 113,983 words (dropped entries with >1 sense and definition >250 chars). 6 routing paths (no COMMENTARY). Same brain + KJV + Strong's + WordNet. For devices that can't open 87MB HTML. |
