...
| `TRU_APEX5.html` | 99.3 MB | 85,252 | **Current canonical.** APEX4 + dictionary fallback: 'what is X' and bare-word queries check WordNet dict before GAP. 'what is a monkey' now returns DEFINE (was GAP). 59/60 audit pass (only '2+2' returns CALC, which is correct). |
