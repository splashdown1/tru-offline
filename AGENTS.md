...
| `TRU_APEX8.html` | 87.1 MB | 63,337 | **Current canonical.** APEX7 + (1) speak() strips HTML before TTS (fixes voice cutout on DEFINE/SCRIPTURE — was reading `<span>` tags aloud); (2) 76 curated Bible facts added to DOCTRINE (66 book authorship + theology/history: KJV, Tyndale, canon, shortest verse, languages, timeline). New queries: 'who wrote genesis', 'what is the kjv', 'when was the bible written', etc. || `TRU_APEX9.html` | 87.1 MB | 63,337 | **Current canonical. Polysemic reasoning engine (PaRDeS).** APEX8 + polysem() function: projects queries across 4 sense-layers — PESHAT (plain/literal), REMEZ (hint/typology), DERASH (applied/doctrine), SOD (mystical/Christological). REMEZ_MAP: 25 typological pairs (lamb→Christ, passover→cross, temple→body, etc.). SOD_PATTERNS: 20 resonance patterns (trinity, incarnation, atonement, resurrection, etc.). Completion synthesis: how senses form a whole. Wired into route override before doctrine. 7/7 polysemic queries pass, 0 regressions. |
...
| `TRU_APEX8_BUNDLE.zip` | 44.7 MB | — | **Zip bundle (under 50 MB).** Contains: TRU_APEX8.html (full, 87 MB) + TRU_APEX_LITE.html (45 MB) + mh_condensed.json (31 MB Matthew Henry commentary) + bible_facts.json (76 facts). For platforms with 50 MB file limits. || `TRU_APEX_MAX.zip` | 49.3 MB | 63,337 | **Max-content build (zipped).** TRU_APEX8 with 6-author commentary layer replacing single MH: Matthew Henry + Barnes + Wesley + Jamieson-Fausset-Brown + John Gill + Spurgeon (Treasury of David). 38,597 commentary entries (106.3 MB). Uncompressed: 163.4 MB. All data layers: brain (63k nodes) + WordNet dict (148k words) + KJV + Strong's + 6-author commentary. 7 routing paths. 11/13 test pass (2 correct doctrine-first). || `TRU_APEX_MAX_POLYSEM.zip` | 49.3 MB | 63,337 | APEX_MAX + polysemic PaRDeS layer. 6-author commentary + polysem() function (PESHAT/REMEZ/DERASH/SOD). || `TRU_APEX_ULTIMATE.zip` | 93.0 MB | 63,337 | **Maximum content build (zipped, under 100 MB).** TRU_APEX9 (polysemic) + mega data: 10-author commentary (MH+Barnes+Wesley+JFB+Gill+Spurgeon+Clarke+Calvin+Keil-Delitzsch+Expositor's) + 70k church-father quotes + 15k reference-dict entries (Easton's, Hastings, Schaff-Herzog) + 5k sermons (MacDonald, Whitefield, Wesley, Luther) + 5k Naves topical + 3k catechism Q&As. Uncompressed: 294.7 MB. 238 MB condensed data across 171k entries. Same 7-path routing + polysemic PaRDeS engine. |
