...
> This is the roadmap. The build hasn't happened yet. This file is the rules for the next TRU we will be building — and how to play with it when it lands.
...
The new TRU is **multi-file in the workspace, single-file at export.** The modular drop setup (see `README.md`) becomes the way the engine evolves — you drop new files in, the engine rebuilds *itself*, not just the brain.
...
4. **multi-file brain** — modular drops become runtime modules, not just static builds
5. **recursive council** — each voice can sub-call the other two before answering
6. **export-as-state** — your seals, weights, and forks travel inside the exported HTML
