...
// ── brain inverted index + BM25 ──
let BRAIN_IDX=null, DOC_LEN=[], AVG_LEN=1, DF={}, N_DOCS=0;
function tokenize(text){
  return text.toLowerCase().replace(/[^a-z0-9\s]/g," ").split(/\s+/).filter(t=>t.length>1 && !STOP.has(t));
}
function buildIndex(){
  BRAIN_IDX={}; DF={}; DOC_LEN=[]; N_DOCS=BRAIN.length;
  let totalLen=0;
  for(let i=0;i<BRAIN.length;i++){
    const node=BRAIN[i];
    const toks=tokenize((node.k||"")+" "+(node.v||""));
    DOC_LEN[i]=toks.length; totalLen+=toks.length;
    const seen=new Set();
  ...
  AVG_LEN=totalLen/N_DOCS || 1;
}
// BM25 scoring — saturating TF, IDF weighting, length normalization
const BM25_K=1.5, BM25_B=0.75;
function bm25Score(term,docIdx){
  const postings=BRAIN_IDX[term]; if(!postings)return 0;
  let tf=0; for(const i of postings) if(i===docIdx) tf++;
  const idf=Math.log(1+(N_DOCS-(DF[term]||0)+0.5)/((DF[term]||0)+0.5));
  const norm=1-BM25_B+BM25_B*(DOC_LEN[docIdx]/AVG_LEN);
  return idf*(tf*(BM25_K+1))/(tf+BM25_K*norm);
}
function retrieveBrain(q,limit=6,exclude){
  ...
  for(const tok of toks) for(const i of (BRAIN_IDX[tok]||[])){
    ...
    scores[i]=(scores[i]||0)+bm25Score(tok,i);
  }
  // semantic boost: if SEMANTIC ready, blend in cosine scores
  // semantic reranking handled async in route() via semRerank()
  return Object.entries(scores).sort((a,b)=>b[1]-a[1]).slice(0,limit).map(([i,s])=>({node:BRAIN[i],score:s}));
}
// ── semantic layer (transformers.js, progressive enhancement) ──
let SEMANTIC_READY=false, semPipe=null;
let semCache={};
async function initSemantic(){
  try{
    statusEl.textContent="● LOADING SEMANTIC MODEL…";
    const {pipeline}=await import("https://esm.sh/@huggingface/transformers@3.0.0");
    semPipe=await pipeline("feature-extraction","Xenova/all-MiniLM-L6-v2",{quantized:true});
    SEMANTIC_READY=true;
    statusEl.textContent="● ONLINE • SEMANTIC + BM25 • CACHED";
    note.textContent=`${BRAIN.length.toLocaleString()} nodes • ${KJV.length.toLocaleString()} verses • semantic ready`;
  }catch(e){ console.warn("semantic layer unavailable, BM25 only",e); statusEl.textContent="● ONLINE • BM25 • OFFLINE"; }
}
async function semEmbed(text){
  if(!semPipe)return null;
  if(semCache[text])return semCache[text];
  const out=await semPipe(text,{pooling:"mean",normalize:true});
  const v=Array.from(out.data); semCache[text]=v; return v;
}
function cosine(a,b){ let dot=0; for(let i=0;i<a.length;i++) dot+=a[i]*b[i]; return dot; }
async function semRerank(q, results, limit){
  if(!SEMANTIC_READY||!semPipe)return results.slice(0,limit);
  const qv=await semEmbed(q);
  if(!qv)return results.slice(0,limit);
  const scored=await Promise.all(results.map(async r=>{
    const nv=await semEmbed((r.node.k||"")+" "+(r.node.v||"").slice(0,200));
    const sim=nv?cosine(qv,nv):0;
    return {node:r.node, score:r.bm25*0.4+sim*0.6, sim, bm25:r.bm25};
  }));
...
async function route(q){
  ...
  let nodes=retrieveBrain(ctxQ,12,exclude); // fetch wider pool for rerank
  // semantic rerank if model ready (async, non-blocking on failure)
  if(SEMANTIC_READY&&nodes.length>3){
    try{nodes=await semRerank(ctxQ,nodes,6);}catch(e){nodes=nodes.slice(0,6);}
  } else { nodes=nodes.slice(0,6); }
  let verdict,reply;
  ...
  const res={reply,verdict,scripture_ref:scripture?scripture.ref:null,
    ...
    follow_up:fu, semantic:SEMANTIC_READY};
  addTurn(q,res);
  return res;
}
...
async function send(qOverride){
  ...
  let res;
  try{res=await route(q);}catch(e){res={reply:"internal fault: "+e.message,verdict:"GAP"};}
...
// async semantic enhancement — loads model, caches in browser, then boosts search
initSemantic();
</script>
</body>
</html>
